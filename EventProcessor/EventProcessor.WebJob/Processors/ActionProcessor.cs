    ...
    public class ActionProcessor : IEventProcessor
    {
        ...
        private async Task ProcessAction(dynamic eventData)
        {
            ...
            try
            {
                ...
                if (ruleOutput.Equals("AlarmBatLevel", StringComparison.OrdinalIgnoreCase))
                {
                    Trace.TraceInformation("ProcessAction: battery level rule triggered!");
                    double batteryLevelReading = ExtractDouble(eventData.reading);

                    string batteryLevelActionId = await _actionMappingLogic.GetActionIdFromRuleOutputAsync(ruleOutput);

                    if (!string.IsNullOrWhiteSpace(batteryLevelActionId))
                    {
                        await _actionLogic.ExecuteLogicAppAsync(
                            batteryLevelActionId,
                            deviceId,
                            "BatteryLevel",
                            batteryLevelReading);
                    }
                    ...
                    {
                        Trace.TraceError("ActionProcessor: batteryLevelActionId value is empty for batteryLevelRuleOutput '{0}'", ruleOutput);
                    }
                }
            }
            catch (Exception e)
