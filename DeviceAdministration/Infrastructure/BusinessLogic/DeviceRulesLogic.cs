...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.DeviceAdmin.Infrastructure.BusinessLogic
{
    ...
    public class DeviceRulesLogic : IDeviceRulesLogic
    {
        ...
        /// <param name="deviceId"></param>
        /// <returns></returns>
        public async Task<DeviceRule> GetNewRuleAsync(string deviceId, string theOperator)
        {
            return await Task.Run(() =>
            {
                var rule = new DeviceRule();
                rule.InitializeNewRule(deviceId, theOperator);
        ...
        {
            foreach (string deviceId in existingDeviceIds)
            {
                ...
                DeviceRule batteryRule = await GetNewRuleAsync(deviceId, "<");
                batteryRule.DataField = DeviceRuleDataFields.BatteryLevel;
                batteryRule.RuleOutput = "AlarmBatLevel";
                batteryRule.Threshold = 15.0d;
                await SaveDeviceRuleAsync(batteryRule);
            }
        }
